...
class mapaWidget(QWidget):
    ...
    def drawTrail(self, painter):
        ...
        final_poses = self.RTVacuum() * np.array([[x], [y], [1], [1]]) * scale
...
if __name__ == "__main__":
    ...
    ymlNode = cfg.getProperty('VacuumSLAM')
    node = rospy.init_node(ymlNode["NodeName"], anonymous=True)

    print("Receiving " + "VacuumSLAM.Pose3D" + " from ROS messages")
    topicP = cfg.getProperty("VacuumSLAM.Pose3D"+".Topic")
