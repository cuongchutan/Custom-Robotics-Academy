...
class LogoWidget(QWidget):
    ...
    def __init__(self,winParent, width=0, height=0):    
        ...
        if (width != 0 and height != 0):
            self.qimage = qimage.scaled(int(0.8*width), int(0.8*height), Qt.KeepAspectRatio)
    ...
    def paintEvent(self, e):
        ...
        painter.drawImage(int(self.width()/2-self.qimage.width()/2), int(self.height()/2-self.qimage.height()/2), self.qimage)
