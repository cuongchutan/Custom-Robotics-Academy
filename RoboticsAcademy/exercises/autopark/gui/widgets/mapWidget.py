...
class MapWidget(QWidget):
    ...
    def drawAxis(self, painter):
        ...
        RT1 = np.array([[0],[0],[0],[1]])
        RT2 = np.array([[200],[0],[0],[1]])
        RT3 = np.array([[0],[200],[0],[1]])
    ...
    def RTx(self, angle, tx, ty, tz):
        RT = np.array([[1, 0, 0, tx], [0, math.cos(angle), -math.sin(angle), ty], [0, math.sin(angle), math.cos(angle), tz], [0,0,0,1]])
        return RT
        
    
    def RTy(self, angle, tx, ty, tz):
        RT = np.array([[math.cos(angle), 0, math.sin(angle), tx], [0, 1, 0, ty], [-math.sin(angle), 0, math.cos(angle), tz], [0,0,0,1]])
        return RT
    
    
    def RTz(self, angle, tx, ty, tz):
        RT = np.array([[math.cos(angle), -math.sin(angle), 0, tx], [math.sin(angle), math.cos(angle),0, ty], [0, 0, 1, tz], [0,0,0,1]])
        return RT   
    ...
    def drawLaser(self, num, painter, color, laser):
        ...
        painter.setPen(pen)
        RT = self.RTLaser(num)
        RTOrigLaser = np.array([[0],[0],[0],[1]]) * self.scale
        ...
        for d in range(0,len(laser)):
            ...
            orig_poses = np.array([[coord[0]], [coord[1]], [1], [1]]) * self.scale
...
class MapWidget1(QWidget):
    ...
    def drawCar(self, painter):
        ...
        orig_poses = np.array([[x], [y], [1], [1]]) * self.scale
    ...
    def drawObstacles(self, painter):
        ...
        # Obstacle 1
        orig_poses1 = np.array([[-13.5], [-3], [1], [1]]) * self.scale
        ...
        # Obstacle 2
        orig_poses2 = np.array([[-7], [-3], [1], [1]]) * self.scale
        ...
        # Obstacle 3
        orig_poses3 = np.array([[0.5], [-3], [1], [1]]) * self.scale
        ...
        # Obstacle 4
        orig_poses4 = np.array([[14], [-3], [1], [1]]) * self.scale
        ...
        # Sidewalk 1
        orig_poses5 = np.array([[5], [9], [1], [1]]) * self.scale
        ...
        # Sidewalk 2
        orig_poses6 = np.array([[5], [-9], [1], [1]]) * self.scale
    ...
    def drawIdeal(self, painter):
        ...
        # Ideal position
        orig_poses = np.array([[7.25], [-3], [1], [1]]) * self.scale
        final_poses = self.RTCar() * orig_poses
