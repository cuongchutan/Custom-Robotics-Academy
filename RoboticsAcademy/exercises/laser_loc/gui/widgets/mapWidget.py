...
class MapWidget(QWidget):
    ...
    def drawRobot(self, painter):
        ...
        pos = self.RTRobot() * np.array([[x], [y], [1], [1]]) * self.scale
    ...
    def drawTrajectory(self, painter):
        ...
        actualx = pose.x
        actualy = pose.y
        actualpos = self.RTRobot() * np.array([[actualx], [actualy], [1], [1]]) * self.scale
    ...
    def drawEstimatedTrajectory(self, painter):
        ...
        for i in range(0,len(est)):
            pos = self.RTRobot() * np.array([[est[i][0]], [est[i][1]], [1], [1]]) * self.scale
            if i == 0:
    ...
    def drawParticles(self, painter):
        ...
        for p in particles: 
            if (p.x > -5 and p.x < 5) and (p.y > -5 and p.y < 5):
                pos = self.RTRobot() * np.array([[p.x], [p.y], [1], [1]]) * self.scale
