...
class LaserWidget(QWidget):
    ...
    def drawRobot(self, painter):
        ...
        final_poses = self.RTRobot() * self.RTz(pi/2, 1, 0, 0)* np.array([[0], [0], [1], [1]]) * self.scale
    ...
    def drawLaser(self, painter, color, laser):
        ...
        painter.setPen(pen)
        RT = self.RTLaser()
        RTOrigLaser = np.array([[0],[0],[0],[1]]) * self.painterScale
        ...
        if (laser == self.laser):
            for d in range(0,len(laser),22):
                ...
                orig_poses = np.array([[coord[0]], [coord[1]], [1], [1]]) * self.painterScale
